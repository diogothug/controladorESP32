...
namespace Modules {
    ...
    void TideEngine::update() {
        ...
        if (prev && next) {
            ...
            if (prev->level < next->level) {
                ...
                // Falling
                currentLevel = prev->level + (next->level - prev->level) * harmonic; // prev > next, so this subtracts
                rising = false;
            }
        } else {
    ...
    bool TideEngine::ensureDataAvailable() {
        ...
        if (daysRemaining < TIDE_UPDATE_THRESHOLD_DAYS) {
            Serial.printf("[TIDE] Low Cache (%.1f days remaining). Requesting Update...\n", daysRemaining);
            needsFetch = true;
            
            // For simulation logic (Mock Fetch)
            if (cache.empty()) fetchMockData(); 
            return false;
        }
    ...
    void TideEngine::fetchMockData() {
        ...
        for (int i = 0; i < (TIDE_CACHE_DAYS + 2) * 4; i++) { // Slightly more than needed
             // Generate ~6h cycle
             TideExtrema ev;
    ...
    float TideEngine::getTideLevel() {
        return currentLevel;
    }

    bool TideEngine::isRising() {
        return rising;
    }
